...
const WhoKnowsAlbumComponents = createPaginationComponentTypes('whoknows-album');
...
> = async ({ interaction, args, statsfmUser, respond }) => {
  ...
  if (!albumIdStr) {
    if (!statsfmUser) {
      await analytics.track('WHO_KNOWS_ALBUM_CURRENTLY_PLAYING_user_not_linked');
      return respond(interaction, {
    ...
    if (!statsfmUser.privacySettings.currentlyPlaying) {
      await analytics.track('WHO_KNOWS_ALBUM_CURRENTLY_PLAYING_user_privacy_currently_playing');
      return respond(interaction, {
        embeds: [
          privacyEmbed(
            interaction.user,
    ...
    if (!currentlyPlaying) {
      await analytics.track('WHO_KNOWS_ALBUM_CURRENTLY_PLAYING_user_not_listening');
      return respond(interaction, {
  ...
  let album: Awaited<ReturnType<typeof api.albums.get>>;
  let data: {
    position: number;
    streams: number;
    playedMs: number;
  ...
  const message = await respond(
    interaction,
    pagination.createMessage<'reply'>(await pagination.current(), WhoKnowsAlbumComponents)
  );

  pagination.manageCollector(message, WhoKnowsAlbumComponents, interaction.user);
